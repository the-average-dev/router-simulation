...
class MetricsCollector:
    ...
    def on_arrival(self, router_id: str, packet, time: float) -> None:
        self.arrivals.append(PacketEvent(
            router_id    = router_id,
            packet_id    = packet.id,
    ...
    def on_enqueue(self, router_id: str, packet, time: float) -> None:
        self.enqueues.append(PacketEvent(
            router_id    = router_id,
            packet_id    = packet.id,
            traffic_type = packet.traffic_type,
            time         = time
    ...
    def on_forward(self, router_id: str, packet, next_hop: str, time: float) -> None:
        self.forwards.append(PacketEvent(
            router_id    = router_id,
            packet_id    = packet.id,
            traffic_type = packet.traffic_type,
            time         = time,
        ...
        logger.debug("FORWARD  pkt=%s router=%s -> %s t=%.4f", packet.id, router_id, next_hop, time)
    ...
    def on_deliver(self, packet, time: float) -> None:
        ...
        self.deliveries.append(PacketEvent(
            router_id    = "DEST",
            packet_id    = packet.id,
            traffic_type = packet.traffic_type,
            time         = time,
